...
namespace JSB.Collections.ConcurrentTrie
{
  public class ConcurrentTrieDictionary<K, V> :  IDictionary<K, V> {
    ...
    class INode<K, V> : INodeBase<K, V> {
      ...
      private MainNode<K, V> gcasRead (ConcurrentTrieDictionary<K, V> ct)
      {
        return GCAS_READ(ct);
      }
      ...
      private INode<K, V> copyToGen ( Gen ngen,  ConcurrentTrieDictionary<K, V> ct)
      {
 ...
 boolean insertln( LNode<K, V> ln,  K k,  V v,  ConcurrentTrieDictionary<K, V> ct)
{
...
private void clean( INode<K, V> nd,  ConcurrentTrieDictionary<K, V> ct, int lev)
{
 ...
 boolean isNullInode( ConcurrentTrieDictionary<K, V> ct)
{
  return GCAS_READ(ct) == null;
}

 int cachedSize( ConcurrentTrieDictionary<K, V> ct)
{
...
public String string (int lev) {
    ...
    private static  class FailedNode<K, V> extends MainNode<K, V> {
  MainNode<K, V> p;

  FailedNode ( MainNode<K, V> p) {
    this.p = p;
    WRITE_PREV(p);
  }
...
private static  class SNode<K, V> extends BasicNode implements KVNode<K, V> {
   K k;
   V v;
   int hc;

  SNode ( K k,  V v,  int hc) {
    this.k = k;
    this.v = v;
    this.hc = hc;
  }
   ...
   public Map.Entry<K, V> kvPair()
{
 ...
 public String string (int lev) {
    ...
    private static  class TNode<K, V> extends MainNode<K, V> implements KVNode<K, V> {
   K k;
   V v;
   int hc;

  TNode( K k,  V v,  int hc) {
    this.k = k;
    this.v = v;
    this.hc = hc;
  }
 ...
 public int cachedSize(Object ct)
{
 ...
 public String string (int lev) {
    ...
    private  static class LNode<K, V> extends MainNode<K, V> {
   ListMap<K, V> listmap;
...
MainNode<K, V> removed(K k,  ConcurrentTrieDictionary<K, V> ct)
{
...
static <K, V> MainNode<K, V> dual( SNode<K, V> x, int xhc,  SNode<K, V> y, int yhc, int lev, Gen gen)
{
  if (lev < 35)
  {
